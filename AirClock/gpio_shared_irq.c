...
/********************************** Q **********************************************/
#define GPIO_SHARED_QUEUE_CAPACITY 32
typedef struct {
    pfnPCU_IRQ_Handler_t        cb;
    uint32_t                    un32Event;
    void                        *pContext;
...
static queue_t s_GSI_Q;                                             // Q
static GPIOSHAREDIRQ_MSG_t s_GSI_QBuf[GPIO_SHARED_QUEUE_CAPACITY];  // Q buf
...
static void PRV_SharedIRQ_Handler(uint32_t un32Event, void *pContext)
{
    ...
    for (int nPin = 0; nPin < PCU_PIN_ID_MAX; nPin++)
    {
        ...
        for (int nPort = 0; nPort < PCU_ID_MAX; nPort++)
        {
            ...
            {
                GPIOSHAREDIRQ_MSG_t m = { s_GSI_Context[nPort][nPin].cb, un32Event, pContext };
                queue_push(&s_GSI_Q, &m);
...
{
    GPIOSHAREDIRQ_MSG_t msg;

    while (queue_pop(&s_GSI_Q, &msg))
    {
        if (msg.cb)
        {
            msg.cb(msg.un32Event, msg.pContext);
        }
    }
}
void EX_SharedIRQ_Init(void)
{
    queue_init(&s_GSI_Q, s_GSI_QBuf, GPIO_SHARED_QUEUE_CAPACITY, sizeof(GPIOSHAREDIRQ_MSG_t));
