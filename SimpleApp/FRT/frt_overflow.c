...
#if (CONFIG_APP_FRT == 1) && (FRT_APP == FRT_APP_MODE_OVERFLOW)
...
static void PRV_FRT_IRQHandler(uint32_t un32Event, void *pContext)
{
    ...
    {
        LOG("[FRT] (%d) O evt fired\n", ptContext->eId);
    }
}

void FRT_Overflow(void)
{
    ...
    FRT_CFG_t tCfg = {
        .eMode = FRT_MODE_FREERUN,
        .eIntr = FRT_INTR_OVERFLOW,
    };

    LOG("[FRT] Overflow interrupt\n");
