...
/* 
 * Readme:
 * This example demonstrates how to use CRC in CRC-16 USB mode.
...
void CRC_16_USB(void)
{
    ...
    eErr = HAL_CRC_SetCompute(CRC_ID_0, un32Init, s_un8Data, sizeof(s_un8Data), &un32Result);
    if (eErr != HAL_ERR_OK)
    {
    ...
    {
        LOG("CRC-16 USB Result mismatch(0x%x, 0x%x)\n", un32Result, CRC_RESULT);
    }
    else
    {
        LOG("CRC-16 USB Result (0x%x)\n", un32Result);
    }
}
#endif
